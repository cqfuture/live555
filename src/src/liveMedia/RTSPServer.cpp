...
void RTSPServer::RTSPClientSession::handleRequestBytes(int newBytesRead)
{
	...
	do 
	{
		...
		ptr[newBytesRead] = '\0';
		fprintf( stderr, 
				"RTSPClientSession[%p]::handleRequestBytes() %s %d new bytes:%s\n",
				this, 
		...
		if (fClientOutputSocket != fClientInputSocket) 
		{
			...
			if (numBytesToDecode > 0) 
			{
				...
				unsigned char* decodedBytes = 
						base64Decode( (char const*)(ptr-fBase64RemainderCount), decodedSize );
		#ifdef DEBUG
				fprintf( stderr, 
			...
			// because we know that we have more input bytes still to receive
...
void RTSPServer::RTSPClientSession::handleCmd_PLAY(ServerMediaSubsession* subsession, char const* cseq,
		   char const* fullRequestStr) {
...
void UserAuthenticationDatabase::addUserRecord(
										char const* username,
										char const* password ) 
{
