...
public: 
	...
	// Note: 该处将类的声明放在类的内部是为了限定类的作用域，同时可解决类与类之间的
	//       头文件包含关系
	...
	protected:
		...
		virtual void handleHTTPCmd_StreamingGET( 
									char const* urlSuffix, 
									char const* fullRequestStr
									);
	protected:
...
protected:
	...
	virtual RTSPClientSession* createNewClientSession(
													unsigned sessionId, 
													int clientSocket, 
													struct sockaddr_in clientAddr 
													);
